// Package diagnostics provides a simple error and warning accumulator with path tracking.
...
func New(operation string) *Diagnostics {
...
func (d *Diagnostics) AddError(code, message, component string, metadata ...map[string]interface{}) {
	...
	logging.Errorf("[%s] %s (Component: %s)", code, message, component)
}
...
func (d *Diagnostics) AddWarning(code, message, component string, metadata ...map[string]interface{}) {
	...
	// Log the warning immediately
	logging.Warnf("[%s] %s (Component: %s)", code, message, component)
}
