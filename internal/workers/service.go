// Package workers provides a service for processing messages
package workers
...
// WorkerMessage represents a message for the service (non-generic version)
...
type ServiceWorker interface {
	...
	Process(ctx context.Context, message *WorkerMessage) error
}
...
// SystemWorkerMessageService is a singleton service that manages message processing
...
var (
	instance *SystemWorkerMessageService
	once     sync.Once
)
...
func GetInstance() *SystemWorkerMessageService {
	if instance == nil {
...
func (s *SystemWorkerMessageService) getPollInterval() time.Duration {
...
func (s *SystemWorkerMessageService) getRecoveryEnabled() bool {
...
func (s *SystemWorkerMessageService) getMaxProcessingAge() time.Duration {
...
func (s *SystemWorkerMessageService) getRecoveryInterval() time.Duration {
...
func (s *SystemWorkerMessageService) getCleanupEnabled() bool {
...
func (s *SystemWorkerMessageService) getCleanupMaxAge() time.Duration {
...
func (s *SystemWorkerMessageService) getCleanupInterval() time.Duration {
...
func (s *SystemWorkerMessageService) getDefaultMaxRetries() int {
...
func (s *SystemWorkerMessageService) RegisterWorker(worker ServiceWorker) error {
	s.workersMu.Lock()
...
func (s *SystemWorkerMessageService) Start(ctx context.Context) error {
	s.workersMu.Lock()
	defer s.workersMu.Unlock()
...
func (s *SystemWorkerMessageService) performStartupRecovery(ctx context.Context) error {
	if s.store == nil {
...
func (s *SystemWorkerMessageService) Stop(ctx context.Context) error {
	s.workersMu.Lock()
	defer s.workersMu.Unlock()
...
func (s *SystemWorkerMessageService) IsRunning() bool {
	s.workersMu.RLock()
...
func (s *SystemWorkerMessageService) GetRegisteredWorkers() []*WorkerMetadata {
	s.workersMu.RLock()
	defer s.workersMu.RUnlock()
...
func (s *SystemWorkerMessageService) listenForMessages(ctx context.Context) {
	if s.store == nil {
...
func (s *SystemWorkerMessageService) dispatchMessage(ctx context.Context, dbMsg *entities.Message) {
	s.workersMu.RLock()
	defer s.workersMu.RUnlock()
...
func (s *SystemWorkerMessageService) updateMessageStatus(ctx context.Context, messageID, status string) {
	if s.store == nil {
		logging.Error("Cannot update message status: MessageDataStore is not initialized")
		return
	}

	messageStatus := entities.MessageStatus(status)
	var err error

	switch messageStatus {
	case entities.MessageStatusCompleted:
...
func (s *SystemWorkerMessageService) PostMessage(ctx context.Context, messageType string, payload interface{}, tenantID string, priority int) (string, error) {
	if s.store == nil {
		return "", fmt.Errorf("MessageDataStore is not initialized")
	}
...
func (s *SystemWorkerMessageService) PostSimple(ctx context.Context, messageType string, payload string, tenantID string) (string, error) {
