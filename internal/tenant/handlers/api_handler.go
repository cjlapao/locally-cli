// Package handlers contains the API handlers for the tenant service“
package handlers
...
func NewApiHandler(tenantService interfaces.TenantServiceInterface) *APIHandler {
	return &APIHandler{
...
func (h *APIHandler) HandleGetTenant(w http.ResponseWriter, r *http.Request) {
	...
	tenant, getDiag := h.tenantService.GetTenantByIDOrSlug(ctx, id)
...
func (h *APIHandler) HandleCreateTenant(w http.ResponseWriter, r *http.Request) {
	...
	request, validateDiag := api.ParseAndValidateBody[tenant_models.TenantCreateRequest](r)
...
func (h *APIHandler) HandleUpdateTenant(w http.ResponseWriter, r *http.Request) {
	...
	request, validateDiag := api.ParseAndValidateBody[tenant_models.TenantUpdateRequest](r)
	if validateDiag.HasErrors() {
		api.WriteErrorWithDiagnostics(w, r, http.StatusBadRequest, "Invalid request body", "Invalid request body", validateDiag)
		return
	}
