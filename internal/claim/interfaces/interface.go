...
type ClaimServiceInterface interface {
	...
	GetClaimsByFilter(ctx *appctx.AppContext, tenantID string, filter *filters.Filter) (*api.PaginatedResponse[pkg_models.Claim], *diagnostics.Diagnostics)
	...
	GetClaimUsers(ctx *appctx.AppContext, tenantID string, claimID string, pagination *pkg_models.Pagination) (*api.PaginatedResponse[pkg_models.User], *diagnostics.Diagnostics)
